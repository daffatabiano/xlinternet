...
import { getPackagesForSitemap } from "@/lib/seo-utils";

// Revalidate setiap 1 jam
export const revalidate = 3600;

/**
 * Sitemap untuk Packages/Paket Internet
 * Endpoint: /api/sitemap-packages
 * Dibatasi max 50,000 URLs per sitemap
 */
export async function GET(): Promise<Response> {
  try {
    const packages = await getPackagesForSitemap();
