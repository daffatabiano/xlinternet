...
export default function EditBlogPage() {
  ...
  /* Pre-fill form when post data arrives */
  useEffect(() => {
    if (!post) return;
    reset({
      title:          post.title ?? '',
      ...
      category:       post.category?.slug ?? post.category ?? 'tutorial',
    ...
    const existingTags = Array.isArray(post.tags)
      ...
      : [''];
    setTags(existingTags.length ? existingTags : ['']);

    // Existing featured image preview
    if (post.featuredImage) {
      setImagePreview(post.featuredImage);
    }
  }, [post, reset]);

  /* Auto-generate slug from title (only when user edits title) */
  const titleValue = watch('title');
  const [titleDirty, setTitleDirty] = useState(false);
  useEffect(() => {
    if (titleDirty && titleValue) {
      setValue('slug', toSlug(titleValue), { shouldValidate: true });
    }
  }, [titleValue, titleDirty, setValue]);

  /* ── Update mutation ───────────────────────────────────────────────────── */
  const { mutate, isPending } = useMutation({
    mutationFn: (data: FormData) => blogService.update(id, data),
    onSuccess: () => {
        ...
        <div className="max-w-3xl space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="space-y-2">
              <div className="h-5 w-48 bg-neutral-200 rounded-lg animate-pulse" />
    ...
    <AdminLayout>
      <div className="max-w-3xl">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="font-display font-bold text-xl text-neutral-900">Edit Artikel</h2>
            <p className="text-sm text-neutral-500">Perbarui konten artikel blog</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-neutral-200 p-6 space-y-5">
            ...
            <Input
              label="Judul Artikel"
              placeholder="Panduan Memilih Paket Internet Terbaik"
              error={errors.title?.message}
              {...register('title', {
                onChange: () => { if (!titleDirty) setTitleDirty(true); },
              })}
            />
          ...
          <div className="bg-white rounded-2xl border border-neutral-200 p-6">
            <label className="flex items-center gap-2.5 cursor-pointer">
              ...
              <span className="text-sm font-medium text-neutral-700">Publikasikan Artikel</span>
            </label>
