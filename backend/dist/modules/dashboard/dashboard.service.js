...
let DashboardService = class DashboardService {
    ...
    async getStats() {
        ...
        const [totalProviders, totalPackages, totalBlogPosts, totalReviews, pendingReviews, newReviewsThisMonth, topProviders, totalLeads, newLeadsToday, convertedLeads, totalPageViews,] = await Promise.all([
    ...
    async getAnalytics(period) {
        ...
        const [totalProviders, totalPackages, totalReviews, totalBlogPosts, totalLeads, convertedLeads, newLeadsInPeriod, recentReviews, recentLeads,] = await Promise.all([
            this.prisma.provider.count({ where: { isActive: true } }),
            this.prisma.package.count({ where: { isActive: true } }),
            this.prisma.review.count({ where: { isApproved: true } }),
            this.prisma.blogPost.count({ where: { isPublished: true } }),
